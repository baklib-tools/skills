...
| [llm-wiki](skills/llm-wiki/SKILL.md)                                   | `npx skills add baklib-tools/skills --skill llm-wiki`                  | 通过 LLM 构建持续进化的个人知识库（Karpathy 方法）：raw/wiki/schema 三层架构，Ingest / Query / Lint 三种核心操作                                                                                                         |
...
| [baklib-theme-dev](skills/baklib-theme-dev/SKILL.md)                       | `npx skills add baklib-tools/skills --skill baklib-theme-dev`            | **Baklib 站点模板**：Liquid 目录与命名、对象/指令/过滤器、静态页、seeds/migrations；**新建主题脚手架**与**参考站复刻工作流**已并入同技能 **`references/`**（`create-theme-scaffold.md`、`website-cloning-workflow.md`） |
