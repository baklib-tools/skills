...
| [software-dev-requirements-lifecycle](skills/software-dev-requirements-lifecycle/SKILL.md) | `npx skills add baklib-tools/skills --skill software-dev-requirements-lifecycle` | 通用软件交付：**文档真源**、需求接收与变更闭环、用户故事与验收、排期与 ADR、**用户帮助中心**目录与导航；**多步执行前须与用户确认**拟办步骤及可选/高风险环节；与 Baklib / 具体技术栈无绑定；**[`reference.md`](skills/software-dev-requirements-lifecycle/reference.md)** 速查、**[`README.md`](skills/software-dev-requirements-lifecycle/README.md)** 安装说明                                                                                                                                 |
