...
## 第 0 轮 — 执行前确认（强制，技能正文要求）

**用户**（示意）：

> 这是我的需求：[粘贴或引用材料]。请按 `requirements-to-published-content` 先列出你准备做的步骤，并问我：要不要对外软文、要不要公众号稿/HTML、要不要**调用 API 出图**（不是只写提示词）、要不要 MCP 写入。

**AI 应答要点**：

- 输出**编号步骤清单**（与技能「执行前确认」一致）。  
- **逐项提问**或列表请用户勾选；**未确认前**不调用计费出图、不写软文、不写入 MCP。  
- 用户确认范围后，再进入分析与方案；若用户只要「分析 + 方案」，则后续轮次全部跳过。

**说明**：下文「第 1 轮」起为**示例仓库成稿**时的分步演示（为便于对照 artifacts **假设用户已确认走满全流程且同意在示例路径落盘**）。真实使用须**始终**先做本轮。
